...
export default class EmployeesServiceFirebase implements EmployeeServiceObservable {
    ...
    async addEmployee(empl: Employee): Promise<any> {
        if (await this.exists(empl.id)) {
            throw ErrorTypes.SERVER_ERROR  //Перебивает ожидаемый тип возвращаемого значения?
        }
    ...
    getAllEmployees(): Observable<Employee[]> {
        return collectionData<Employee>(this.db) //что я принимаю (Типизация как Props)
            .pipe(map((employees) => {
