...
export default class AuthServiceFirebase implements AuthService {
    ...
    registration(loginData: LoginData): Promise<LoginCodes> {
    ...
    private socialNetworkAuth(providerName: string): Promise<LoginCodes> {
        ...
        return appFirebase.auth().signInWithPopup(authProvider)
            .then(() => LoginCodes.OK)
            .catch(() => LoginCodes.WRONG_CREDENTIALS);
    }
    ...
    private static getProvider(providerName: string): firebase.auth.AuthProvider | undefined  {
        ...
        switch (providerName) {
            case "google": res =new firebase.auth.GoogleAuthProvider(); break
            case "twiter": res = new firebase.auth.TwitterAuthProvider(); break
            case "facebook": res = new firebase.auth.FacebookAuthProvider(); break
